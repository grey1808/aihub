...
class KnowledgeSourceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Сразу забираем документы: отдельный шаг «а теперь нажмите
        // синхронизировать» люди пропускают, и источник остаётся пустым.
        // Если путь или доступ указаны неверно, ошибка попадёт в историю
        // обновлений и будет видна в карточке.
        SyncSourceJob::dispatch($source->id);

        return redirect()
            ->route('admin.sources.edit', $source)
            ->with('status', 'Источник создан, документы загружаются. '
                            .'Обновите страницу через минуту и посмотрите историю обновлений внизу.');
    }
